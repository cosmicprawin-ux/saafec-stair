...
def load_double_mutation_delimited(path: Path) -> dict[str, DoubleMutationProteinRecord]:
    ...
    with path.open(encoding="utf-8") as handle:
        ...
        mutation1_col = _resolve_column(reader.fieldnames, ["mutation_1", "mutation1", "first_mutation"])
        mutation2_col = _resolve_column(reader.fieldnames, ["mutation_2", "mutation2", "second_mutation"])
        ...
        if (mutation1_col is None or mutation2_col is None) and double_mutation_col is None and (
            ...
            raise ValueError(
                f"{path} needs either mutation_1/mutation_2, double_mutation, "
