...
def _resolve_required_columns(fieldnames: list[str]) -> dict[str, str]:
    ...
    expanded_aliases = {
        ...
        "mut_aa": ["mut_aa", "mt", "mutant", "mutantaa"],
    }
    try:
        resolved = _resolve_columns(fieldnames, structure_aliases)
    except ValueError as exc:
        raise ValueError(
            "Mutations CSV is missing required columns. "
            f"{exc} Every supported schema requires ['pdb', 'chain']."
        ) from exc
    ...
    mutation_column = next(
        (
            normalised_to_actual[_normalise_column_name(alias)]
            for alias in ["mut", "mutation", "substitution"]
            if _normalise_column_name(alias) in normalised_to_actual
        ),
        None,
    )
    expanded: dict[str, str] = {}
    for logical_name, candidates in expanded_aliases.items():
        for candidate in candidates:
            actual = normalised_to_actual.get(_normalise_column_name(candidate))
            if actual is not None:
                expanded[logical_name] = actual
                break

    if mutation_column is not None:
        resolved["mutation"] = mutation_column
    if len(expanded) == len(expanded_aliases):
        resolved.update(expanded)
    elif expanded:
        raise ValueError(
            "Expanded single-mutation columns must be supplied together as "
            "['position', 'wt_aa', 'mut_aa']."
        )
    if "mutation" not in resolved and "position" not in resolved:
        raise ValueError(
            "Mutations CSV needs either a compact 'mut' column such as A10V "
            "or the expanded columns ['position', 'wt_aa', 'mut_aa']."
        )
    return resolved
...
def _parse_compact_mutation(value: object, *, row_num: int) -> tuple[str, int, str]:
    token = str(value or "").strip().upper()
    match = COMPACT_MUTATION_RE.match(token)
    if match is None:
        raise ValueError(
            f"Row {row_num}: invalid mutation {value!r}; expected a compact "
            "substitution such as A10V."
        )
    wt_aa, position, mut_aa = match.groups()
    position_1based = int(position)
    if position_1based < 1:
        raise ValueError(f"Row {row_num}: mutation positions must be one-based and positive.")
    return wt_aa, position_1based, mut_aa
...
def _dict_reader(handle: Any, path: Path) -> csv.DictReader:
    return mutation_list_reader(handle, path)
    ...
    *,
    ...
    with open(csv_path, encoding="utf-8") as fh:
        ...
        for row_num, row in enumerate(reader, start=2):
            ...
            compact_values: tuple[str, int, str] | None = None
            if "mutation" in column_map:
                compact_values = _parse_compact_mutation(
                    row.get(column_map["mutation"]),
                    row_num=row_num,
                )

            expanded_values: tuple[str, int, str] | None = None
            if "position" in column_map:
                try:
                    position_1based = int(row[column_map["position"]])
                except ValueError as exc:
                    raise ValueError(
                ...
                if position_1based < 1:
                    raise ValueError(f"Row {row_num}: mutation positions must be one-based and positive.")
                expanded_values = (
                    row[column_map["wt_aa"]].strip().upper(),
                    position_1based,
                    row[column_map["mut_aa"]].strip().upper(),
                )

            if compact_values is not None and expanded_values is not None and compact_values != expanded_values:
                raise ValueError(
                    f"Row {row_num}: mutation representations conflict: compact "
                    f"{compact_values[0]}{compact_values[1]}{compact_values[2]} versus "
                    f"expanded {expanded_values[0]}{expanded_values[1]}{expanded_values[2]}."
                )

            wt_aa, position_1based, mut_aa = compact_values or expanded_values  # type: ignore[misc]
