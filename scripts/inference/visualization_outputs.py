...
from inference.prediction_csv import format_final_ddg, write_prediction_rows
    ...
    *,
    ...
    for pos_idx, wt_aa in enumerate(sequence):
        ...
        for aa in AA_ORDER:
            ...
            row[f"{aa}_{DELTA_DELTA_G_COLUMN_LABEL}"] = value
        rows.append(row)
    return rows
...
def write_all_mutation_matrix_csv(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    columns = [
        *((field, field) for field in CSV_PREFIX_COLUMNS),
        *((f"{aa}_{DELTA_DELTA_G_COLUMN_LABEL}", aa) for aa in AA_ORDER),
    ]
    display_rows = []
    for row in rows:
        display_row = dict(row)
        for aa in AA_ORDER:
            field = f"{aa}_{DELTA_DELTA_G_COLUMN_LABEL}"
            display_row[field] = format_final_ddg(row[field])
...
def write_visualization_outputs(
    *,
    ...
    if write_visualizations:
        from visualization.create_ddg_heatmap import (  # noqa: WPS433
    ...
    for protein_name, entry in ensemble_matrices.items():
        ...
        if write_visualizations:
            ...
            rows, meta = build_matrix_payload(
                matrix_rows,
                CSV_PREFIX_COLUMNS + [f"{aa}_{DELTA_DELTA_G_COLUMN_LABEL}" for aa in AA_ORDER],
                matrix_path.name,
            )
