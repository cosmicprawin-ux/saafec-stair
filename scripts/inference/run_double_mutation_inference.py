...
DEFAULT_INPUT_TABLE = f"{DATASET_ROOT}/double_mutation_list.txt"
...
def save_prediction_rows(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    field_map = [
        ...
        ("mt_aa_1", "mt_aa_1"),
        ("mt_aa_2", "mt_aa_2"),
        ("additive_single_baseline", "additive_single_baseline_DDG"),
        ("calibrated_additive_prior", "calibrated_additive_prior_DDG"),
        ("calibration_delta", "calibration_delta_DDG"),
        ("contact_prior", "contact_prior_DDG"),
        ("residual_gate", "residual_gate"),
        ("interaction_residual", "interaction_residual_DDG"),
        ("total_correction", "total_correction_DDG"),
        ("_prediction_separator", ""),
        ("prediction", "Predicted_DDG"),
    ]
    display_rows = [
        {**row, "prediction": format_final_ddg(row["prediction"])}
        for row in rows
    ]
    with path.open("w", newline="", encoding="utf-8") as handle:
        write_prediction_rows(handle, field_map, display_rows)
