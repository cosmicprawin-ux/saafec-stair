...
def read_matrix(csv_path: Path) -> tuple[list[dict], dict]:
    with csv_path.open(newline="") as handle:
        ...
        if first_row and first_row[0].strip() == "Unit(DDG)=kcal/mol":
            reader = csv.DictReader(handle)
        elif first_row and first_row[0].strip() == "Unit":
            if len(first_row) < 2 or first_row[1].strip() != "kcal_per_mol":
                raise ValueError(f"Unsupported prediction unit row in {csv_path}")
            reader = csv.DictReader(handle)
        else:
            handle.seek(0)
            reader = csv.DictReader(handle)
    ...
    return build_matrix_payload(rows, list(reader.fieldnames or []), csv_path.name)
...
def build_matrix_payload(
    ...
    """Build heatmap data from full-precision rows or a displayed CSV."""

    if not rows:
        raise ValueError(f"No rows found in {source_csv}")

    aa_columns = {
        aa: aa_delta_delta_g_column(aa, fieldnames)
        for aa in AA_ORDER
    }
        ...
        payload_rows.append(
            {
                ...
                "insertionCode": row.get("insertion_code", ""),
                "wildType": wt,
