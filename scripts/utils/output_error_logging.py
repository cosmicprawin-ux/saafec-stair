...
-----------------------
Shared tagged error logging for the SAAFEC-STAIR inference pipeline.

Errors and warnings are both printed to the console and appended to
step-specific folders under `output/errors/` so failures remain
...
def infer_output_base(output_path: str | Path, default_work_dir: Path) -> Path:
    """
    Infer the shared `output/` root from a script-specific output path.
    """
