...
----------------------------------------
Cache per-residue SaProt embeddings for SAAFEC-STAIR inference.
...
protein in the same by_protein layout consumed by ``MutationDataset``.
"""
...
from inference.prepare_single_pdb_input import (  # noqa: E402
    PDBCompletenessError,
    find_internal_missing_residues,
    format_missing_residue_error,
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", default="output/saprot_embeddings")
    parser.add_argument(
        "--saprot-model-dir",
        default="SaProt",
        help=(
...
def load_saprot_masked_lm(model_dir: Path, model_cls: Any) -> tuple[torch.nn.Module, str]:
    ...
    through `from_pretrained` because of CVE-2025-32434. Local SaProt weights
    are expected to come from the documented upstream source; fall back to a
    direct state-dict load only for that specific safety gate and only from a
    resolved local path.
    """
    try:
...
def main() -> None:
    ...
    try:
        ...
        for protein_name in protein_names:
            ...
            candidate_stems = [structure_id, protein_name]
            if chain_id and structure_id and not str(structure_id).endswith(f"_{chain_id}"):
                candidate_stems.append(f"{structure_id}_{chain_id}")
            pdb_path = None
            for candidate_stem in candidate_stems:
                pdb_path = (
                    available_pdbs.get(candidate_stem)
                    or available_pdbs_lower.get(str(candidate_stem).lower())
                )
        ...
        for job in protein_jobs:
            missing_residues = find_internal_missing_residues(job["pdb_path"], job["chain_id"])
            if missing_residues:
                completeness_errors.append(
                    format_missing_residue_error(job["pdb_path"], job["chain_id"], missing_residues)
                )
        if completeness_errors:
            if len(completeness_errors) == 1:
                raise PDBCompletenessError(completeness_errors[0])
            raise PDBCompletenessError(
                "Refusing mutation-stability prediction because one or more input PDBs have "
                "unresolved internal residues.\n\n"
                + "\n\n---\n\n".join(completeness_errors[:10])
                + (
                    ""
                    if len(completeness_errors) <= 10
                    else f"\n\n... and {len(completeness_errors) - 10} more incomplete PDB(s)."
                )
            )
        ...
        for job in protein_jobs:
            ...
            try:
                missing_residues = find_internal_missing_residues(pdb_path, chain_id)
                if missing_residues:
                    raise PDBCompletenessError(
                        format_missing_residue_error(pdb_path, chain_id, missing_residues)
                    )
                ...
                print(f"[done] {protein_name} -> {work_path_str(run_dir)}")
...
if __name__ == "__main__":
    try:
        main()
    except PDBCompletenessError as exc:
