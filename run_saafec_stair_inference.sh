...
run_single() {
  ...
  local input_table="${INPUT_TABLE:-$ROOT_DIR/examples/quickstart/single_mutation/single_mutations.txt}"
  ...
  if [[ -n "${INPUT_PDB:-}" && -z "${INPUT_TABLE:-}" ]]; then
    ...
    echo "Single mode needs INPUT_PDB=/path/to/protein.pdb or INPUT_TABLE plus PDB_DIR." >&2
    echo "Default INPUT_TABLE: $input_table" >&2
  ...
  if [[ -n "${SPECIFIED_MUTATION_TABLE:-}" ]]; then
    ...
    python scripts/inference/write_specified_single_mutation_predictions.py \
      --specified-table "$SPECIFIED_MUTATION_TABLE" \
  ...
  write_text_output_tables "$out_root"
...
run_double() {
  local input_table="${INPUT_TABLE:-$ROOT_DIR/examples/quickstart/double_mutation/double_mutations.txt}"
  local table_sheet
  ...
  if [[ ! -f "$input_table" || ! -d "$pdb_dir" ]]; then
    echo "Double mode needs INPUT_TABLE plus PDB_DIR." >&2
    echo "Default INPUT_TABLE: $input_table" >&2
    echo "Default PDB_DIR:     $pdb_dir" >&2
    exit 2
  fi
  ...
  echo "Double-mutation predictions text: $(display_path "$prediction_dir")/mutation_DDG_predictions.txt"
