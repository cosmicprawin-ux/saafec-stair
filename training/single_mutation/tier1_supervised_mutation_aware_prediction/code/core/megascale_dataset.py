...
def _resolve_mutation_model_position(
    ...
    if MUTATION_INDEXING_MODE == "native_reference_exact":
        idx, method, reason = _resolve_mutation_model_position_reference_parser(
            mutation,
            wt_sequence,
            parser_context,
        ...
        if idx is not None:
            return idx, method, reason
        attempts.append(reason or method)
    ...
    if workbook_sequence:
        ...
        sequence_position = mutation.get("position_seq") or raw_position
        try:
            sequence_position = int(sequence_position)
        except (TypeError, ValueError):
            sequence_position = raw_position
        if 1 <= sequence_position <= len(workbook_sequence):
            if workbook_sequence[sequence_position - 1] == wt_aa:
                ...
                mapped_idx = ref_to_model[sequence_position - 1]
                if mapped_idx is not None and wt_sequence[mapped_idx] == wt_aa:
                    method = (
                        "workbook_sequence_alignment_mut_pos_seq"
                        if sequence_position != raw_position
                        else "workbook_sequence_alignment"
                    )
                    return mapped_idx, method, None
                attempts.append("workbook_sequence_alignment:wt_mismatch_or_gap")
            else:
                attempts.append(
                    "workbook_sequence_alignment:reference_wt_mismatch_"
                    f"saw_{workbook_sequence[sequence_position - 1]}_expected_{wt_aa}"
                )
