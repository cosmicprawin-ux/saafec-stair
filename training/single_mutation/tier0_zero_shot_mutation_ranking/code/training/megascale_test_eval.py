...
def build_mutation_prediction_rows_for_protein(
    *,
    ...
    """Build metric-level mutation rows from the exact masked entries used in evaluation."""
    ...
    for pos_idx, mut_idx in masked_positions.tolist():
        ...
        ddg_values = [
            _format_float(record.get("ddg"))
            for record in source_records
            if record.get("ddg") not in {None, ""}
        ]
        rows.append(
            {
                ...
                "source_ddg_values": ";".join(ddg_values),
