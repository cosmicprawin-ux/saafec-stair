...
from core.megascale_dataset import MegaScaleDataset, ProteinSample
...
class SaProtProteinMPNNDataset(Dataset[SaProtProteinMPNNSample]):
    """Wrap ``MegaScaleDataset`` with a paired ProteinMPNN logits cache."""
...
def saprot_proteinmpnn_collate_fn(samples: list[SaProtProteinMPNNSample]) -> dict[str, Any]:
    ...
    targets = torch.zeros(batch_size, max_len, 20, dtype=torch.float32)
    masks = torch.zeros(batch_size, max_len, 20, dtype=torch.float32)
    lengths: list[int] = []
    sequences: list[str] = []
    names: list[str] = []
    ...
    for i, sample in enumerate(samples):
        ...
        targets[i, :length] = sample.target.float()
        masks[i, :length] = sample.mask.float()
