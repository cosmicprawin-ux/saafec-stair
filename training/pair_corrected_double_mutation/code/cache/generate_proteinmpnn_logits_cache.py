...
from core.saafec_pdb_chain_parser import (  # noqa: E402
    AA3_TO_1,
...
from models.stability_head import AMINO_ACIDS_20  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Pinned ThermoMPNN protein_mpnn_utils.py supplied outside this repository.",
    )
...
def load_proteinmpnn(
    *,
    ...
    print(
        f"Loaded ProteinMPNN checkpoint {ckpt_path} "
...
def sequence_jobs_from_csv(table_path: Path) -> list[ProteinMPNNCacheJob]:
    with table_path.open(encoding="utf-8") as fh:
        ...
        protein_col = _resolve_optional_column(reader.fieldnames, ["protein_name", "protein", "proteinname"])
        structure_col = _resolve_optional_column(reader.fieldnames, ["pdb", "structure_id", "structure", "pdb_id"])
        chain_col = _resolve_optional_column(reader.fieldnames, ["chain", "pdb_chain_id", "chain_id"])
        ...
        if protein_col is None:
            ...
            raise ValueError(f"Could not resolve protein column in {table_path}: {reader.fieldnames}")
        by_name: dict[str, Counter[str]] = {}
        structure_by_name: dict[str, str] = {}
        chain_by_name: dict[str, str | None] = {}
        for row in reader:
            protein_name = str(row.get(protein_col, "")).strip()
            if not protein_name:
                continue
            by_name.setdefault(protein_name, Counter())
            if structure_col is not None and protein_name not in structure_by_name:
                structure_id = str(row.get(structure_col, "")).strip()
                if structure_id:
                    structure_by_name[protein_name] = structure_id
            if chain_col is not None and protein_name not in chain_by_name:
                chain_id = str(row.get(chain_col, "")).strip()
                chain_by_name[protein_name] = chain_id or None
    ...
    for protein_name, sequence_counts in sorted(by_name.items()):
        ...
        jobs.append(
            ProteinMPNNCacheJob(
                protein_name=protein_name,
                structure_id=structure_by_name.get(protein_name, protein_name),
                chain_id=chain_by_name.get(protein_name),
                sequence=sequence,
...
def main() -> None:
    ...
    resolved_dataset, loaded_jobs = load_sequence_jobs(args.dataset_csv, args.xlsx_sheet)
    ...
    summary = {
        ...
        "proteinmpnn_checkpoint": args.proteinmpnn_checkpoint,
        "proteinmpnn_checkpoint_sha": checkpoint_sha,
